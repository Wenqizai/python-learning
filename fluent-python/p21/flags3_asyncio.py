...
async def get_country(client: httpx.AsyncClient, cc: str) -> str:
    # http://mp.ituring.com.cn/files/flags/af/metadata.json
    url = f"{BASE_URL}/flags/{cc}/metadata.json".lower()
    resp = await client.get(url, timeout=3.1, follow_redirects=True)
    resp.raise_for_status()
    metadata = resp.json()
...
async def download_one(client: httpx.AsyncClient, 
    ...
    try:
        ...
        filename = country.replace(' ', '_') + '.gif'
